        ...
        columns:[
          ...
          },
          {
            ...
            render:function(data,type,full,meta){
              ...
              return  `${date.toLocaleDateString('en-GB', options)} <br/> <span class="status-text ${!!parseInt(full?.isApplicant) ? 'green' : 'red'}">${parseInt(full?.isApplicant) ? 'Applicant' : 'Not Applicant'}</span>`;
            }
          },
          {
            data: 'action',
            ...
            render:function(data,type,full,meta){
              var editUrl =
                ...
                .replace(':id', full.id);
              var addApplicantUrl =!!parseInt(full?.isApplicant) ? `{{route('admin.applicant.edit',['id' =>':id'])}}`.replace(':id',full?.applicant_id)  : `{{route('admin.applicant.create')}}?passport_id=${full.id}`;
              var editButton =
                '<a title="Edit" class="primary-btn" href="' + editUrl + '"><i class="fa fa-pencil"></i></a>';
              var addAppButton = `<a title='${parseInt(full?.isApplicant) ? 'Update Applicant': "Add Applicant"}' class="primary-btn" href="${addApplicantUrl}"><i class='${!!parseInt(full?.isApplicant) ? 'fa fa-user' : 'fa fa-user-plus'}'></i></a>`
              var actionButtons =
